...
//		this->initialize(CN_BUFFER_SIZE);
	...
	void realloc_data() {
		...
		if(this->data + this->data_size != this->data_end) {
			print("Data end set incorrectly");
		}
	}
...
//			print("Error get_long(): Accessing beyond end of serialized data");
	...
	char get_char() {
		...
		if(this->data_get_ptr > this->data_end) {
			print("Error get_char(): Accessing beyond end of serialized data");
		}
		return *c;
	}
...
//		print("Fetching",len);
		for(int i=0;i<len;i++) {
			*s += this->get_char();
			if(this->data_get_ptr >= this->data_end) {
//				print("get_str(): Reached end of data");
				return true;
			}
		}
		return false;
...
//				print("get_data(): Reached end of data");
	...
	bool test_ptrs() {
		if(data != data_set_ptr) {
			print("Data set pointer set incorrectly");
			return false;
		}
		if(data_get_ptr != data_set_ptr) {
			print("Data get pointer set incorrectly");
			return false;
		}
		if(this->data + this->data_size != this->data_end) {
			print("Data end set incorrectly");
