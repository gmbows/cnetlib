...
makestr += execname+": "+" ".join(binlist)+"\n\tg++  "+" ".join(binlist)+" "+libs+" -lSDL2main -lSDL2 -lSDL2_image -lSDL2_ttf  -fpermissive -O3 -o "+execname+"\n\n"
...
for src in srcs:
	...
	if src in heads:
		head = src+".h"
	deps = getDeps(src)
	makestr += "bin/{1}.o: {0}.cpp {2} \n\tg++  -c {0}.cpp ".format(src,src[src.rfind('/')+1:],head + deps)+libs+" -fpermissive -lSDL2main -lSDL2 -lSDL2_image -lSDL2_ttf -o bin/{1}.o\n\n".format(src,src[src.rfind('/')+1:],head + deps)
