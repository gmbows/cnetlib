...
makestr += execname+": "+" ".join(binlist)+"\n\tC:\Qt\Tools\mingw1120_64\\bin\g++  "+" ".join(binlist)+" "+libs+" -L /lib -D_WIN32_WINNT=0x0601 -lws2_32 -lwsock32 -lmingw32 -fpermissive -O3 -o "+execname+"\n\n"
...
for src in srcs:
	...
	if src in heads:
		head = src+".h"
	deps = getDeps(src)
	makestr += "bin/{1}.o: {0}.cpp {2} \n\tC:\Qt\Tools\mingw1120_64\\bin\g++  -c {0}.cpp -lpthread -fpermissive -O3 -L /lib -lmingw32  -D_WIN32_WINNT=0x0601 -lws2_32 -lwsock32 -o bin/{1}.o\n\n".format(src,src[src.rfind('/')+1:],head + deps)
